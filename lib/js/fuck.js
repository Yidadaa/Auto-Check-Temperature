...
function fakeMintUI() {
  var fakeProxy = () => new Proxy(function (cb) {
    if (arguments.length && typeof (cb) !== 'function') {
      NMSL.postMessage('[mintUI]: ' + Array.from(arguments).map(v => v.toString()).join(', '))
    }
...
function fuckDailyReport(debug = true) {
  ...
  var save = () => new Promise((resolve) => {
    this.$router = {
      go() {
        NMSL.postMessage('[auto fuck dailyReportInfo] done.')
        resolve()
      }
    };
...
function fuckTemp(time = 0, debug = true) {
  ...
  const initData = () => new Promise(resolve => {
    ...
    NMSL.postMessage('[体温信息] ' + tempFormData.DAY_TIME_DISPLAY)
    tempFormData.WID = '';
  ...
  const save = () => new Promise(resolve => {
    this.$router = {
      go() {
        NMSL.postMessage('[auto fuck saveMyTempReportDetail] done.')
        resolve()
      }
    };

    NMSL.postMessage('[温度]: ' + this.tempReportInfo.TEMPERATURE)
    NMSL.postMessage('[数据]: ' + JSON.stringify(this.tempReportInfo))
...
function fuckAll(debug = true) {
  fuckTemp(0, debug).then(res => {
    NMSL.postMessage("status:1")
    fuckTemp(1, debug).then(res => {
      NMSL.postMessage("status:2")
      fuckTemp(2, debug).then(res => {
        NMSL.postMessage("status:3")
        fuckDailyReport(debug).then(res => {
          NMSL.postMessage("status:4")
        })
      })
    })
  })
